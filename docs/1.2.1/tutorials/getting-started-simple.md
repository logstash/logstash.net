...
[download it
now](https://download.elasticsearch.org/logstash/logstash/logstash-1.2.1-flatjar.jar).
...
Save this to a file called `logstash-simple.conf` and run it like so:
    ...
    java -jar logstash-1.2.1-flatjar.jar agent -f logstash-simple.conf
...
Now run it all (again. Be sure to stop your previous Logstash tests!):
    ...
    java -jar logstash-1.2.1-flatjar.jar agent -f logstash-complex.conf -- web
