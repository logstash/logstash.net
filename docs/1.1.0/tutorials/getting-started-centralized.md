...
Follow [this link to download logstash-1.1.0](http://semicomplete.com/files/logstash/logstash-1.1.0-monolithic.jar).
...
Put this in a file and call it 'myshipper.conf' (or anything, really), and run: 
    ...
    java -jar logstash-1.1.0-monolithic.jar agent -f myshipper.conf
...
argument. 
    ...
    % java -jar logstash-1.1.0-monolithic.jar web
...
specify the host explicitly using the --backend flag:
    ...
    % java -jar logstash-1.1.0-monolithic.jar web --backend elasticsearch://myserver/
