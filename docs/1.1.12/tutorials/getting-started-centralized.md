...
Follow [this link to download logstash-1.1.12](https://download.elasticsearch.org/logstash/logstash/logstash-1.1.12-flatjar.jar).
...
Put this in a file and call it 'shipper.conf' (or anything, really), and run: 
    ...
    java -jar logstash-1.1.12-flatjar.jar agent -f shipper.conf
...
`java -jar logstash-1.1.12-flatjar.jar agent -f indexer.conf`
...
argument. 
    ...
    java -jar logstash-1.1.12-flatjar.jar web --backend elasticsearch://127.0.0.1/
