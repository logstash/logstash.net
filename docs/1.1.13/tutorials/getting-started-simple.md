...
[download it
now](https://download.elasticsearch.org/logstash/logstash/logstash-1.1.13-flatjar.jar).
...
Logstash ships with a built-in web interface. It's fairly spartan but it's a good proof-of-concept. Let's restart our logstash process with an additional option:
    ...
    java -jar logstash-1.1.13-flatjar.jar agent -f logstash-simple.conf -- web --backend elasticsearch://localhost/
...
Now run it all (again. Be sure to stop your previous Logstash tests!):
    ...
    java -jar logstash-1.1.13-flatjar.jar agent -f logstash-complex.conf -- web --backend elasticsearch://localhost/
