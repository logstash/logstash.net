...
include the plugin right in the jar file.
    ...
    % jar -uf logstash-1.2.0-flatjar.jar logstash/filters/foo.rb

    # Verify it's in the right location in the jar!
    % jar tf logstash-1.2.0-flatjar.jar | grep foo.rb
    logstash/filters/foo.rb

    % java -jar logstash-1.2.0-flatjar.jar agent -f example.conf
