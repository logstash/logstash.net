...
You should download the logstash 'monolithic' jar - if you haven't yet, [download it now](http://semicomplete.com/files/logstash/logstash-1.1.1-monolithic.jar). This package includes most
...
Now run it all (again. Be sure to stop your previous Logstash tests!):
    ...
    java -jar logstash-1.1.1-monolithic.jar agent -f logstash-complex.conf -- web --backend elasticsearch:///?local
