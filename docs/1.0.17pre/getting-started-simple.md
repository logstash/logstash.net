...
You should download the logstash 'monolithic' jar - if you haven't yet, [download it now](http://semicomplete.com/files/logstash/logstash-1.0.17pre-monolithic.jar). This package includes most
...
Now run it all:
    ...
    java -jar logstash-1.0.17pre-monolithic.jar agent -f mylogstash.conf -- web --backend elasticsearch:///?local
