...
(logstash-1.0.15.jar, etc) Once you have grok installed, you need to install the
...
Put this in a file and call it 'myshipper.conf' (or anything, really), and run: 
    ...
    java -jar logstash-1.0.15-monolithic.jar agent -f myshipper.conf
...
argument. 
    ...
    % java -jar logstash-1.0.15-monolithic.jar web
...
specify the host explicitly using the --backend flag:
    ...
    % java -jar logstash-1.0.15-monolithic.jar web --backend elasticsearch://myserver/
