...
Follow [this link to download logstash-1.1.6](https://download.elasticsearch.org/logstash/logstash/logstash-1.1.6-monolithic.jar).
...
Put this in a file and call it 'shipper.conf' (or anything, really), and run: 
    ...
    java -jar logstash-1.1.6-monolithic.jar agent -f shipper.conf
...
`java -jar logstash-1.1.6-monolithic.jar agent -f indexer.conf`
...
argument. 
    ...
    java -jar logstash-1.1.6-monolithic.jar web --backend elasticsearch://127.0.0.1/
