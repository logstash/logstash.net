...
You should download the logstash 'monolithic' jar - if you haven't yet, [download it now](https://download.elasticsearch.org/logstash/logstash/logstash-1.1.6-monolithic.jar). This package includes most
...
Save this to a file called `logstash-simple.conf` and run it like so:
    ...
    java -jar logstash-1.1.6-monolithic.jar agent -f logstash-simple.conf
...
Logstash ships with a built-in web interface. It's fairly spartan but it's a good proof-of-concept. Let's restart our logstash process with an additional option:
    ...
    java -jar logstash-1.1.6-monolithic.jar agent -f logstash-simple.conf -- web --backend elasticsearch:///?local
...
Now run it all (again. Be sure to stop your previous Logstash tests!):
    ...
    java -jar logstash-1.1.6-monolithic.jar agent -f logstash-complex.conf -- web --backend elasticsearch:///?local
