...
----
Now you should have the file named 'logstash-1.4.1-modified.tar.gz' on your local filesystem. Let's unpack it:
----
...
NOTE: This tutorial specifies running Logstash 1.4.1-modified with Elasticsearch 1.1.1. Each release of Logstash has a *recommended* version of Elasticsearch to pair with. Make sure the versions match based on the http://logstash.net/docs/latest[Logstash version] you're running!
