...
- you can include the plugin right in the jar file.
    ...
    % jar -uf jar -uf logstash-1.1.2.dev-monolithic.jar logstash/filters/foo.rb

    # Verify it's in the right location in the jar!
    % jar tf logstash-1.1.2.dev-monolithic.jar | grep foo.rb
    logstash/filters/foo.rb

    % java -jar logstash-1.1.2.dev-monolithic.jar agent -f example.conf
