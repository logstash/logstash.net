...
Follow [this link to download logstash-1.1.2](http://semicomplete.com/files/logstash/logstash-1.1.2-monolithic.jar).
...
Put this in a file and call it 'shipper.conf' (or anything, really), and run: 
    ...
    java -jar logstash-1.1.2-monolithic.jar agent -f shipper.conf
...
`java -jar logstash-1.1.2-monolithic.jar agent -f indexer.conf`
...
argument. 
    ...
    java -jar logstash-1.1.2-monolithic.jar web --backend elasticsearch://127.0.0.1/
