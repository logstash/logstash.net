...
for easy downloading of ElasticSearch:
    ...
    ES_PACKAGE=elasticsearch-0.19.10.zip
...
Follow [this link to download logstash-1.1.2.dev](http://semicomplete.com/files/logstash/logstash-1.1.2.dev-monolithic.jar).
...
Put this in a file and call it 'shipper.conf' (or anything, really), and run: 
    ...
    java -jar logstash-1.1.2.dev-monolithic.jar agent -f shipper.conf
...
`java -jar logstash-1.1.2.dev-monolithic.jar agent -f indexer.conf`
...
argument. 
    ...
    java -jar logstash-1.1.2.dev-monolithic.jar web --backend elasticsearch://127.0.0.1/
